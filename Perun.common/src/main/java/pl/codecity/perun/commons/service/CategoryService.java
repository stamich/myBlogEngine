package pl.codecity.perun.commons.service;
...
import pl.codecity.perun.commons.configuration.PerunCacheConfiguration;
import pl.codecity.perun.commons.helper.CategorySearchRequest;
import pl.codecity.perun.commons.model.Category;
import pl.codecity.perun.commons.repository.CategoryRepository;
...
//	@Inject
	...
	@CacheEvict(value = {PerunCacheConfiguration.ARTICLE_CACHE, PerunCacheConfiguration.PAGE_CACHE}, allEntries = true)
		...
		for (int i = 0; i < data.size(); i++) {
			...
			if (map.get("item_id") != null) {
				...
				if (category != null) {
					...
					category.setParent(parent);
					category.setLeft(((int) map.get("left")) - 1);
					category.setRight(((int) map.get("right")) - 1);
	...
	public Category deleteCategory(long id, String language) {
		...
		categoryRepository.shiftLftRgt(category.getLeft(), category.getRight());
		categoryRepository.shiftRgt(category.getRight());
		categoryRepository.shiftLft(category.getRight());
